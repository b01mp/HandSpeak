...
#define GESTURE_FLEX_INDEX_THUMB 6
#define GESTURE_FLEX_INDEX_PINKY 7
#define GESTURE_GYRO_RIGHT_THUMB 8
#define GESTURE_GYRO_LEFT_THUMB 9
...
void play_mp3_file(int file_number) {
    if (file_number < 1 || file_number > 10) {
...
void detect_gestures(float gyro_x, float gyro_y, float gyro_z) {
    ...
    if (is_finger_straight(index, 1390, 1430) && is_finger_straight(thumb, 1400, 1450))
    {
        ...
        detected_gesture = GESTURE_FLEX_INDEX_THUMB;
    }
    else if (is_finger_straight(index, 1390, 1430) && is_finger_straight(pinky, 1400, 1450))
    {
        detected_gesture = GESTURE_FLEX_INDEX_PINKY;
    }
    else if (is_finger_straight(thumb, 1400, 1450) && gyro_z > 20)
    {
        detected_gesture = GESTURE_GYRO_RIGHT_THUMB;
    }
    else if (is_finger_straight(thumb, 1400, 1450) && gyro_z < -20)
    {
        detected_gesture = GESTURE_GYRO_LEFT_THUMB;
    }
    // Gesture 1: Index finger flex
    else if (is_finger_straight(index, 1390, 1430)) {
    ...
    else if (is_finger_straight(pinky, 1400, 1450)) {
    ...
    if (detected_gesture != GESTURE_NONE && detected_gesture != current_gesture) {
        ...
        switch (detected_gesture) {
            ...
            case GESTURE_FLEX_INDEX_THUMB:
                play_mp3_file(6);
                ESP_LOGI(TAG, "Gesture: Thumb + Index Flex - Playing 'This is Yash'");
                break;

            case GESTURE_FLEX_INDEX_PINKY:
                play_mp3_file(7);
                ESP_LOGI(TAG, "Gesture: Pinky + Index Flex - Playing 'I Hope you liked it' ");
                break;

            case GESTURE_GYRO_RIGHT_THUMB:
                play_mp3_file(8);
                ESP_LOGI(TAG, "Gesture: Thumb + Gyro Right - Playing 'Have a Great Day'");
                break;
            
            case GESTURE_GYRO_LEFT_THUMB:
                play_mp3_file(9);
                ESP_LOGI(TAG, "Gesture: Thumb + Gyro Left 'this is Bhavesh'");
                break;
        }
    }
