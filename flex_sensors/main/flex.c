...
void app_main(void) {
    ...
    while (1) {
        ...
        if (is_finger_straight(index, 1390, 1430) && last_played != 1) {
            play_mp3_file(1);  // Good Morning
            last_played = 1;
        }
        else if (is_finger_straight(thumb, 1400, 1450) && last_played != 2) {
            play_mp3_file(2);  // byebye
            last_played = 2;
        }
        else if (is_finger_straight(middle, 1720, 1780) && last_played != 3) {
            play_mp3_file(3);
            last_played = 3;  // goodnight
        }
        else if (is_finger_straight(ring, 1710, 1745) && last_played != 4) {
            play_mp3_file(4);
            last_played = 4;   // thankkyou
        }
        else if (is_finger_straight(pinky, 1200, 1250 /*&& last_played != 5*/)) {
            play_mp3_file(5);
            last_played = 5;   //hello
        }
        else if (!is_finger_straight(thumb, 1400, 1450) &&
                 !is_finger_straight(index, 1390, 1430) &&
                 !is_finger_straight(middle, 1770, 1825) &&
                 !is_finger_straight(ring, 1710, 1745) &&
                 !is_finger_straight(pinky, 1000, 1200
            )) {
